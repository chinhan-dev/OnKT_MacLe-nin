...
# Tracking covered UIDs for Sections 1, 2, 4, 5
...
sec5_1, sec5_2, sec5_3, sec5_4, sec5_5 = [], [], [], [], []
...
# HIGH-PRECISION LOOPOLE DEFINITIONS
loopholes_def = [
    {
        'id': 1,
    ...
    },
    {
        ...
        'check': lambda q: ("thực tại khách quan" in q['opts'][q['correct']].lower() or "độc lập với ý thức" in q['opts'][q['correct']].lower() or "tồn tại khách quan" in q['opts'][q['correct']].lower()) and "bổ sung" not in q['q'].lower()
    },
    {
        'id': 4,
        ...
        'check': lambda q: "lênin bổ sung" in q['q'].lower() or "lênin phát triển" in q['q'].lower() or "độc quyền" in q['opts'][q['correct']].lower()
    },
    {
        'id': 5,
        ...
        'check': lambda q: "chất phác" in q['q'].lower() or "chất phác" in q['opts'][q['correct']].lower()
    },
    {
        'id': 6,
...
# Build HTML with Interactive Dropdown Table
html = f"""<!DOCTYPE html>
<html lang="vi">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Mẹo Học & Quy Tắc Khoanh Trắc Nghiệm Triết Học Mác - Lênin (Chuẩn Xác 100%)</title>
...
<body>
  ...
  <nav class="tab-bar">
    ...
    <button class="tab-item" data-sec="sec3">
      ...
      <span>3: Bảng Lỗ Hổng & Sổ Câu Hỏi</span>
      <small>Click xem ngay</small>
    </button>
...
html += f"""
  ...
  <!-- TAB 3 CONTENT: BẢNG LỖ HỔNG CÂU HỎI CLICK SỔ CÂU HỎI TRỰC TIẾP -->
  <div class="tab-content" id="sec3">
    <div class="section">
      <div class="section-title">
        <i class="fa-solid fa-bolt" style="color: #d97706;"></i>
        PHẦN 3: BẢNG LỖ HỔNG CÂU HỎI (BẤM VÀO HÀNG ĐỂ SỔ XUẤT TOÀN BỘ CÂU HỎI LIÊN QUAN)
      </div>
...
for lh in loopholes_def:
    lh_id = lh['id']
    count = len(lh['matched'])
    grid_html = render_grid_section(lh['matched'])
    
    html += f"""
          <!-- LOOPHOLE ROW {lh_id} -->
          <tr class="lh-row" data-target="lh-details-{lh_id}">
            <td class="q-col">{lh['q_label']}</td>
            <td class="a-col">
              {lh['a_label']}
              <span class="toggle-badge"><i class="fa-solid fa-chevron-down"></i> Xem {count} câu</span>
            </td>
          </tr>
          <!-- EXPANDED DETAILS ROW -->
          <tr class="lh-details-row" id="lh-details-{lh_id}">
            <td colspan="2" class="lh-details-cell">
              <div class="details-title">
                <i class="fa-solid fa-folder-open" style="color: #2563eb;"></i>
                DANH SÁCH {count} CÂU HỎI CHUẨN XÁC: "{lh['q_label']}"
              </div>
              {grid_html}
            </td>
          </tr>
...
html += f"""
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {{
      ...
      // Accordion Table Rows Toggle
      const lhRows = document.querySelectorAll('.lh-row');
      lhRows.forEach(row => {{
        row.addEventListener('click', () => {{
          const targetId = row.dataset.target;
          const targetDetailsRow = document.getElementById(targetId);
          const icon = row.querySelector('.toggle-badge i');

          if (targetDetailsRow) {{
            const isExpanded = targetDetailsRow.classList.contains('expanded');
            
            if (isExpanded) {{
              targetDetailsRow.classList.remove('expanded');
              row.classList.remove('expanded');
              if (icon) icon.className = 'fa-solid fa-chevron-down';
            }} else {{
              targetDetailsRow.classList.add('expanded');
...
print("Successfully generated high-precision interactive meo_hoc.html!")
