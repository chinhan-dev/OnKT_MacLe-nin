...
rules = [
    ...
    },
    {
        ...
        'check': lambda q: ("thực tại khách quan" in q['correct_txt'].lower() or "độc lập với ý thức" in q['correct_txt'].lower()) and "bổ sung" not in q['q'].lower()
    },
    {
        'id': 4,
    ...
    },
    {
        ...
        'check': lambda q: "vận động" in q['q'].lower() and ("phương thức tồn tại" in q['correct_txt'].lower() or "tuyệt đối" in q['correct_txt'].lower() or "vật chất" in q['correct_txt'].lower())
    },
    {
        'id': 7,
    ...
    },
    {
        ...
        'check': lambda q: "ý thức" in q['q'].lower() and ("phản ánh" in q['correct_txt'].lower() or "sáng tạo" in q['correct_txt'].lower() or "chủ quan" in q['correct_txt'].lower()) and "ý thức xã hội" not in q['q'].lower()
    },
    {
        'id': 9,
        'title': '9. Đề hỏi vai trò của "Thực tiễn"',
        'check': lambda q: "thực tiễn" in q['q'].lower() and ("cơ sở" in q['correct_txt'].lower() or "động lực" in q['correct_txt'].lower() or "mục đích" in q['correct_txt'].lower() or "tiêu chuẩn" in q['correct_txt'].lower())
    },
    {
        'id': 10,
        'title': '10. Đề hỏi "3 Phát minh Khoa học Tự nhiên" tiền đề Mác',
        'check': lambda q: "khoa học tự nhiên" in q['q'].lower() or "phát minh" in q['q'].lower() or "tế bào" in q['correct_txt'].lower() or "đácuyn" in q['correct_txt'].lower()
    },
    {
        'id': 11,
        'title': '11. Đề hỏi quan hệ "Quyết định" (Vật chất / Hạ tầng / Kinh tế)',
        'check': lambda q: "quyết định" in q['q'].lower() and ("quyết định" in q['correct_txt'].lower() or "cơ sở" in q['correct_txt'].lower())
    },
    {
        'id': 12,
...
print("=== FULL PRECISION AUDIT REPORT ===")
for r in rules:
    matched = [item for item in all_q if r['check'](item)]
    print(f"\nRule {r['id']}: [{len(matched)} câu] {r['title']}")
    for m in matched:
        print(f"  - [{m['tab']} Q{m['id']}] Q: {m['q'][:60]}... -> Ans({m['correct_lbl']}): {m['correct_txt']}")
